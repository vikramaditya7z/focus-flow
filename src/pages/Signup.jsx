  ...
  return (
    <div className="flex items-center justify-center min-h-[80vh] my-8">
      <div className="glass-panel p-10 rounded-3xl border border-slate-700/50 shadow-2xl w-full max-w-md relative overflow-hidden">
        ...
        <h1 className="text-3xl font-extrabold text-white mb-2 text-center tracking-tight">Create Account</h1>
        <p className="text-slate-400 text-center mb-8">Start locking in and supercharge productivity.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative">
            ...
            <input type="email" required placeholder="Email Address" value={email} onChange={e => setEmail(e.target.value)}
              className="w-full bg-slate-900/80 border border-slate-700 rounded-2xl py-4 pl-12 pr-4 text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 transition-all font-medium" />
          </div>
          <div className="relative">
          ...
          <button disabled={loading} type="submit" 
            className="w-full flex items-center justify-center py-4 rounded-xl shadow-lg text-lg mt-2 font-bold text-white bg-emerald-600 hover:bg-emerald-500 transition-all active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed">
            {loading ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : <UserPlus className="w-5 h-5 mr-2" />} Sign Up
          </button>
        </form>
        ...
        <p className="mt-8 text-center text-slate-400 font-medium">
          Already have an account? <Link to="/login" className="text-emerald-400 hover:text-emerald-300 ml-1 hover:underline">Log in</Link>
        </p>
      </div>
    </div>
  );
